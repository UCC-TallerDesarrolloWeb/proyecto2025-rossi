...
const agregarAlCarrito = (nombre, precioBase, imagen, idMedallones, idPapas, idBebida) => {
    ...
    const producto = {
        ...
        papas,
        bebida,
        precioFinal: calcularPrecio(precioBase, medallones, papas, bebida)
    };
...
const eliminarProducto = (id) => {
    guardarCarrito(obtenerCarrito().filter((producto) => producto.id !== id));
    mostrarCarrito();
};
 ...
 * @returns {void}
 */
const invalidarCampo = (campo, mensaje) => {
    alert(mensaje);
    campo.value = '';
    campo.focus();
};
...
const validarFormulario = (evento) => {
    ...
    if (!campos.nombre.value.trim()) {
        invalidarCampo(campos.nombre, 'Ingresá tu nombre.');
        return;
    }
    if (!campos.apellido.value.trim()) {
        invalidarCampo(campos.apellido, 'Ingresá tu apellido.');
        return;
    }
    if (campos.telefono.value.trim() && !/^[0-9\s()-]+$/.test(campos.telefono.value)) {
        invalidarCampo(campos.telefono, 'El teléfono solo puede contener números, espacios, guiones o paréntesis.');
        return;
    }
    if (!campos.correo.validity.valid) {
        invalidarCampo(campos.correo, 'Ingresá un correo electrónico válido.');
        return;
    }
    if (!campos.mensaje.value.trim()) {
        invalidarCampo(campos.mensaje, 'Ingresá un mensaje.');
        return;
    }
